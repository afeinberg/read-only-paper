...
and rollback. \citet{percolator} takes a hybrid approach that's
between a traditional parallel RDBMS and a combination of a batch
processing (e.g., Map/Reduce) and a distributed key-value store,
implemented on top of BigTable~\cite{bigtable}; unlike
\projectname{}, cross-row and cross-table ACID transactions are used
for incremental updates.


More importantly, since the data itself is immutable,
\projectname{} makes optimizations at the storage layer
level. 
