...
Community Edition version~5.0.27. 
...
\sql{LOAD DATA INFILE} command. This ignores the time it took to convert
...
\sql{ALTER TABLE...DISABLE KEYS} statement before the load. 
...
alloted a huge buffer size. On running a simple I/O benchmark during
the build we notice that MySQL does around 1.4 times more data output
compared our implementation. The optimized B$^{+}$, on the other hand,
...
\begin{figure}
  ...
  \caption{The time to complete the build with varying input data size. We terminated the MySQL test early due to prolong data load time} 
  \label{build}
\end{figure}
...
faster cache warming process. Also all numbers presented for read-only
storage engine after this use binary search.  

\begin{figure}
  \centering
    \includegraphics[scale=0.55]{images/search_1node.pdf}
  \caption{The single node median read latency taken at 1 minute intervals since the swap. The slope of the graph shows the rate of cache warming}
  \label{search}
\end{figure}
...
\begin{figure}
  ...
  \caption{Read latency vs varying data size for 16 node cluster}
  \label{16search}
\end{figure}
...
\begin{figure*}
  ...
  \caption{Read latency vs time since swap, for PYMK and CF}
  \label{production}
\end{figure*}
