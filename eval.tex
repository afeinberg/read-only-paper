...
\sql{LOAD DATA INFILE} command. This ignores the time it took convert
...
\begin{figure}
  ...
  \caption{Read Latency vs time since swap, showing rate of cache warming}
  \label{search}
\end{figure}
...
the amount of queries per second while maintaining serving latency. 

\begin{figure}
  \centering
    \includegraphics[scale=0.55]{images/mysql_vs_read.pdf}
  \caption{Read Latency vs varying throughput}
  \label{mysql:search}
\end{figure}
